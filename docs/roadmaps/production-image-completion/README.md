...
See [the tracker](./01-implementation-tracker.md) for the only authoritative slice status. `S00` through `S02` and `S03.1a` are verified; `S03.1b` is the active slice.
